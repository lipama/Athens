...
public class Modules<M extends Module> {
    ...
    private void load() {
        autoRespawn.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, autoRespawn.name());
        crystalAura.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, crystalAura.name());
        fullBright.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, fullBright.name());
        autoFish.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, autoFish.name());
        autoFarm.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, autoFarm.name());
        boatFly.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, boatFly.name());
        flight.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, flight.name());
        xRay.enabled = SaveUtils.loadState(SaveUtils.SavableData.MODULE, xRay.name());
    }
    public void tick() {
        int drawInt = 5;
